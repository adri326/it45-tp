...
struct nn_t build_nearest_neighbor_sub(double* dist, size_t n_towns) {
    ...
    for (size_t n = 1; n < n_towns; n++) {
        ...
        for (size_t j = 1; j < n_towns; j++) {
            ...
            if (j == current || dist[current * n_towns + j] < 0)
            ...
            if (dist[current * n_towns + j] < candidate_score ||
                candidate == -1) {
                candidate = j;
...
`dist[y * n_towns + x]` is the distance from city `y` to city `x`.
    @arg n_towns - the number of towns; `dist` should have as size `n_towns *
n_towns`.
