...
void check_nn_solution(struct nn_t* solution, int* expected, double* dist, size_t n_cities) {
    ...
    ck_assert_double_eq_tol(solution->evaluation, sum, 0.000001 * sqrt(n_cities));
}
...
/** @test test_nearest_neighbor_random
    Robust testing of the `build_nearest_neighbor_sub` function, using a set of `n` cities placed around a circle.
    The order of the cities is shuffled between two rounds, alongside the radius and phase of the regular polygon.
    Two solutions are possible for each iteration, given the symmetries of the polygon. `check_nn_solution` takes care of this.
**/
START_TEST(test_nearest_neighbor_random) {
    ...
    for (size_t n_cities = 5; n_cities < 20; n_cities++) {
        for (size_t sample = 0; sample < n_cities * n_cities / 4; sample++) {
            ...
            // Fill with 0..n
            for (size_t n = 0; n < n_cities; n++) order[n] = (int)n;
            ...
            for (size_t n = 0; n < n_cities; n++) {
                coords[n][0] = radius * cos(M_PI * 2.0 * (float)order[n] / (float)n_cities + offset);
                coords[n][1] = radius * sin(M_PI * 2.0 * (float)order[n] / (float)n_cities + offset);
            }
